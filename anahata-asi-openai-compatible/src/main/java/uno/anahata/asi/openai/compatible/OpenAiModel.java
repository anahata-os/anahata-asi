...
import uno.anahata.asi.openai.compatible.adapter.OpenAiResponsesApiContentAdapter;
...
public class OpenAiModel extends OpenAiCompatibleModel {
    ...
    public List<String> getSupportedActions() {
        return List.of(prefersResponsesApi() ? "responses" : "chat/completions");
    }
    ...
    private ObjectNode prepareResponsesPayload(GenerationRequest request, boolean stream) {
        ...
        // 1. System Instructions: Use top-level 'instructions' field
        // This keeps SI in the 'Config' JSON and out of the 'History' JSON.
        if (!request.config().getSystemInstructions().isEmpty()) {
            String consolidatedInstructions = String.join("\n\n", request.config().getSystemInstructions());
            payload.put("instructions", consolidatedInstructions);
        }
        ...
        if (localTools != null && !localTools.isEmpty()) {
            ...
            Map<AbstractToolkit, List<AbstractTool>> grouped = localTools.stream().collect(Collectors.groupingBy(AbstractTool::getToolkit));
        ...
        // 4. History (Messages and Tool Calls)
