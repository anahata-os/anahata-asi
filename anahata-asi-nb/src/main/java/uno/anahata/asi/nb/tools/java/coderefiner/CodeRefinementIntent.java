...
/**
 * Represents a single structural AST modification instruction in a flattened
 * format.
 * <p>
 * This intent is processed by the {@link BatchCodeRefiner} to perform surgical,
 * AST-guided text replacements on Java source files.
 * </p>
 */
@Data
@Slf4j
...
public class CodeRefinementIntent implements Serializable {
    ...
    /**
     * Defines the types of structural modifications available for a
     * CodeRefinementIntent.
     */
    public enum Type {
        /**
    ...
    /**
     * The fully qualified name of the target class container. Used for
     * resolving the container during structural inserts. Nested types should
     * use the '$' separator.
     */
    ...
    /**
     * The absolute fully qualified name of the target member to perform the
     * operation on. Supports methods, fields, and inner classes. Method FQNs
     * must include full parameter types (type erasure applied).
     */
    ...
    /**
     * The exact string declaration (signature) of the member. This should
     * include modifiers, return types, and parameter lists but NOT the body or
     * Javadoc.
     */
    ...
    /**
     * The code body or initializer expression. For methods, this is the code
     * inside the braces. For fields, it is the expression following the
     * assignment operator.
     */
    ...
    /**
     * A human-readable reason explaining the intent. Visible in the UI during
     * the refinement review phase.
     */
    ...
    /**
     * The structured Javadoc configuration to apply to the member. Allows for
     * synchronous documentation and code updates.
     */
    ...
    /**
     * Generates a formatted diagnostic string detailing the intent's
     * configuration.
     *
    ...
    public String applyToText(CompilationController cc, String currentContent) throws Exception {
        ...
        if (type == Type.UPDATE) {
            ...
            if (javadoc != null) {
                ...
                if (!newDocStr.endsWith(baseIndent)) {
                    newDocStr += baseIndent;
                }
            }
        ...
        if (type == Type.DELETE || type == Type.MOVE) {
            ...
            int end = (int) endPos;
            while (end < currentContent.length() && (currentContent.charAt(end) == ' ' || currentContent.charAt(end) == '\t' || currentContent.charAt(end) == ';')) {
                end++;
            }
            ...
            if (container == null) {
                throw new AgiToolException("Target container not found: " + classFqn);
            }
        ...
        if (type == Type.INSERT) {
            ...
            if (container instanceof ClassTree ct) {
                ...
                for (Tree m : ct.getMembers()) {
                    if (!cc.getTreeUtilities().isSynthetic(new TreePath(ctPath, m))) {
                        members.add(m);
                    }
                }
            ...
            String declStr = declaration != null ? declaration.stripIndent().trim() : "";
