    ...
    public void validateWrite(AbstractTextFileWrite write) throws Exception {
        ...
        if (fo != null) {
            ...
            if (write.getLastModified() != 0 && current != write.getLastModified()) {
                throw new AiToolException("Optimistic locking failure: lastModified timestam mismatch. \nYou provided: " + write.getLastModified() + ", Current on disk: " + current + "\n. Always use the lastModified from the resource header");
            }
        } else {
