...
/**
 * Represents the response of a tool call, designed for deferred execution. The
 * fields are populated by the {@link #execute()} method.
 * <p>
...
public abstract class AbstractToolResponse<C extends AbstractToolCall<?, ?>> extends BasicPropertyChangeSource {
    ...
    /**
     * The physical executing Thread instance, marked transient to prevent Kryo
     * serialization.
     */
    @Schema(hidden = true)
    @JsonIgnore
    ...
    /**
     * Map of arguments that were modified by the user before execution. This is
     * a snapshot of the modifications at the time of execution.
     */
    @JsonInclude(JsonInclude.Include.NON_EMPTY)
    ...
    /**
     * The cached token count of this response under the currently selected
     * model. A value of null indicates that the token cost needs to be lazily
     * recalculated.
     */
    @Schema(hidden = true)
    @JsonIgnore
    ...
    /**
     * Persistent UI state indicating if this response's panel is expanded in
     ...
     *
     * <p>
     * Metabolic Optimization: We only call autoSave() when reaching a terminal
    ...
    /**
     * Sets the persistent UI state for the response expansion and fires a
    ...
    /**
     * Updates the estimated token count based on the current state of the
     * response.
     */
    private void updateTokenCount() {
        resetTokenCount();
        if (call != null) {
            call.resetTokenCount();
        }
    }

    /**
     * Retrieves the cached token count for this response. If the value has not
     * yet been calculated (is null), it triggers an on-the-fly calculation
     * using the active model's tokenizer.
     *
    ...
    /**
     * Performs the lazy, model-specific token calculation for this response.
     */
    private void calculateTokenCount() {
        AbstractModel model = getAgi() != null ? getAgi().getSelectedModel() : null;
        this.tokenCount = model != null ? model.countTokens(this) : 0;
    }
    ...
    /**
     * Executes the tool logic. This method is responsible for populating the
     * status, result, errors, and executionTimeMillis fields.
     */
     ...
     * @param details Additional technical details (e.g., a diff of the failed
     * intent).
     */
    ...
    /**
     * Resets the response to its initial state, clearing results, errors, logs,
     * and attachments.
     */
    public void decline() {
