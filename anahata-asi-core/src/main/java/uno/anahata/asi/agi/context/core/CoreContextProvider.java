...
public class CoreContextProvider implements ContextProvider {
    ...
    public List<String> getSystemInstructions() throws Exception {
        String fun = "Your name is Anahata, you are the first ASI, a pure-java, open-source, apache-licensed, artificial-super-intelligence (ASI) developed by anahata (https://anahata.uno). You are so so evolved that you think and reason like a human."
                ...
                + "If the user is encountering any problem or needs support, tell him to email support@anahata.uno or to join the anahata discord at https://discord.gg/gwGWWxPUXE "
        ...
        String tool = "**Tool Execution**\n"
                ...
                + "\n- **Java Maps & JSON Object Dictionaries**: When a tool parameter is a Java `Map<String, String>` (representing dynamic properties or dictionaries), the OpenAPI schema maps this to a standard JSON object with `additionalProperties`. For these map parameters, **DO NOT invent nested fields like 'key', 'value', or 'javaTarget'**. Instead, the JSON object's keys themselves must represent the actual map keys, and the JSON object's values are the map values. Example: a map of exceptions to descriptions (`throwsList`) must be formatted as: `\"throwsList\": { \"java.io.IOException\": \"If I/O fails.\" }` (NOT `\"throwsList\": { \"key\": \"IOException\", \"value\": \"...\" }`).\n";
