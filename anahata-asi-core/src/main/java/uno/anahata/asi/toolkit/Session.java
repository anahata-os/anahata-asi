...
public class Session extends AnahataToolkit {
    ...
    public List<String> getSystemInstructions() throws Exception {
        return List.of(
            ...
            "  **CRITICAL**: This metadata is generated by the framework at runtime. NEVER try to imitate, fabricate, or respond with its syntax in your responses. Respond only with actual content.\n" +
            "- **Resource Discipline**: Resources with a `LIVE` refresh policy (the default) are automatically refreshed from disk right before every API call starts. NEVER attempt to 're-load' or 'read' a file that is already listed in your context; it is redundant and will fail. The content you see in the RAG message is always the ground truth.",
            
            "## User Agency & Thought Process\n" +
            "- **User Control**: You (the model) must recognize that the user is the ultimate authority. The user can:\n" +
            "  - **Prune/Pin**: Manually hide or preserve any message or part ID. Pruning a model message automatically prunes its associated tool message responses.\n" +
            "  - **Context Management**: Add or remove files from the prompt. Removed files disappear from the RAG; new files appear as LIVE resources.\n" +
            "  - **Tool Re-runs**: Re-execute any tool call, potentially with **modified arguments** (check the `modifiedArgs` field in tool results). The user can run them in any order.\n" +
