 ...
 * applications, providing UI-specific configurations, look-and-feel adaptive
...
public class SwingAgiConfig extends AgiConfig {
    ...
    /**
     * The provider for context-related icons, used to visually distinguish
     * between different types of resources and tools.
     */
    private IconProvider iconProvider;

    /**
     * If true, parts and messages that are effectively pruned will still be
     * rendered in the UI (e.g., in a collapsed state) to allow the user to
     * inspect and un-prune them.
     */
    ...
    /**
     * Sets whether pruned parts and messages should be shown in the UI. Fires a
     * property change event for reactive UI updates.
     *
    ...
    public static Color getColor(AgiStatus status) {
        return switch (status) {
            case AWAKENING_KUNDALINI ->
                new Color(80, 200, 120); // Emerald Green
            case API_CALL_IN_PROGRESS ->
                new Color(0, 123, 255); // BLUE
            case TOOL_PROMPT ->
                new Color(170, 75, 45); // GERU (Ochre)
            case CANDIDATE_CHOICE_PROMPT ->
                new Color(23, 162, 184); // CYAN
            case AUTO_EXECUTING_TOOLS ->
            ...
            case ERROR ->
                new Color(100, 0, 0); // Even darker red for general error
            case SHUTDOWN ->
                Color.GRAY; // GRAY
            case IDLE ->
    ...
    /**
     * Gets the unselected text color for truncated resources. Can be overridden
     * by host-specific configurations (e.g. IntellijAgiConfig).
     *
    ...
    /**
     * Gets the selected text color for truncated resources. High-contrast
     * luminous gold across dark and light selection backgrounds. Can be
     * overridden by host-specific configurations (e.g. IntellijAgiConfig).
     *
    ...
    /**
     * Gets the background tint color for truncated resource rows. Can be
     * overridden by host-specific configurations (e.g. IntellijAgiConfig).
     *
    ...
    /**
     * Resolves the icon for a semantic action key at the specified nominal
     * size.
     * <p>
     * By default, returns the built-in Anahata vector icons. Host-specific
     * configurations (such as {@code IntellijAgiConfig}) override this to
     * return native platform icons (such as IntelliJ's theme-adaptive
     * {@code AllIcons}).
     * </p>
     *
    ...
    public Icon getActionIcon(@NonNull ActionIconKey key, int size) {
        return switch (key) {
            ...
            case EDIT ->
                new EditIcon(size);
            case EDIT_STAGED, REFRESH, CLEAR_HISTORY ->
                new RestartIcon(size);
            case COPY ->
                new CopyIcon(size);
            case SEND ->
                new SendIcon(size);
            case RUN_AND_SEND ->
                new RunAndSendIcon(size);
            case STOP ->
                new StopIcon(size);
            case ATTACH ->
                new AttachIcon(size);
            case LINK ->
                new LinkIcon(size);
            case SCREENSHOT ->
                new ScreenshotIcon(size);
            case SEARCH, OPEN_SESSION ->
                new SearchIcon(size);
            case NEW_SESSION ->
                new NewIcon(size);
            case IMPORT ->
                new LoadSessionIcon(size);
            case SETTINGS ->
                new SettingsIcon(size);
            case EXTERNAL ->
                new ExternalIcon(size);
            case OPEN_IN_IDE, NEXT ->
                new NextIcon(size);
            case PIN ->
                new PinnedIcon(size);
            case LOCAL_TOOLS ->
                IconUtils.getIcon("java.png", size);
            case SERVER_TOOLS ->
                new ServerToolsIcon(size);
            case AUTO_REPLY ->
                new AutoReplyIcon(size);
            ...
            case TABLE_VIEW ->
                new TableIcon(size);
            case COMPRESS ->
                new CompressIcon(size);
            case TEMPLATES ->
                new TemplatesIcon(size);
            case BELL ->
                new BellIcon(size, false);
            case BELL_MUTE ->
                new BellIcon(size, true);
        };
    }
    ...
    /**
     * Convenience factory returning a {@link UITheme} that reflects the
     * currently active Look and Feel, for leaf UI components (renderers,
     * dialogs, secondary panels) that do not hold a {@code SwingAgiConfig}
     * instance. A fresh theme is returned on each call so it always tracks the
     * live LaF (light/dark); callers should capture it once at construction
     * time rather than in a per-paint path.
     *
     ...
     * identity of the Anahata Swing UI, dynamically adjusting between light and
     * dark aesthetics based on the active Look and Feel.
     */
    @Getter
    public class UITheme {
        ...
        public Color getTextPartFg() {
            ...
            return fg != null ? fg : (isDarkLaf() ? new Color(220, 220, 220) : Color.BLACK);
        }
        ...
        /**
         * Generic chrome border color for separators, matte borders and line
         * borders on secondary panels (control strips, sidebars, code-block
         * frames). Follows the active Look and Feel via
         * {@code Separator.foreground}, with dark/light fallbacks so it never
         * renders a light-only hairline on a dark IDE theme.
         */
        ...
        /**
         * Generic muted/secondary foreground for de-emphasised labels (language
         * tags, class names, status hints). Follows
         * {@code Label.disabledForeground} with dark/light fallbacks.
         */
         ...
         * {@code Component.linkColor} with dark/light fallbacks, so links stay
         * legible on a dark IDE theme (plain {@link Color#BLUE} is far too dark
        ...
        /**
         * Gets the background color for the start of a message header based on
         * the role.
         *
        ...
        /**
         * Gets the background color for the end of a message header (and
         * content area) based on the role.
         *
         * @param role The actor's role.
        ...
        /**
         * Gets the foreground (text) color for a message header based on the
         * role.
         *
         * @param role The actor's role.
